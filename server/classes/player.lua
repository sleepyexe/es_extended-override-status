...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata, status)
    local targetOverrides = Config.PlayerFunctionOverride and Core.PlayerFunctionOverrides[Config.PlayerFunctionOverride] or {}

    local self = {}
    local status = {
        hunger = ESX.Math.Round(status.hunger or 100, 2),
        thirst = ESX.Math.Round(status.thirst or 100, 2),
        stress = ESX.Math.Round(status.stress or 0, 2),
    }
    ...
    if Config.Multichar then
        self.license = Config.Identifier .. identifier:sub(identifier:find(":"), identifier:len())
    else
        self.license = Config.Identifier..":" .. identifier
    end
    ...
    stateBag:set("status", self.status, true)
    stateBag:set('hunger', self.status.hunger, true)
    stateBag:set('thirst', self.status.thirst, true)
    stateBag:set('stress', self.status.stress, true)
    ...
    ---@param name string
    ---@param value number
    function self.setStatus(name, value)
    ...
    ---@param name? string
    ---@return any
    function self.getStatus(name)
        if name and type(name) ~= 'string' then return end
        return name and self.status[name] or self.status
    end

    --- Add Player Status
    ---@param name string
    ---@param value number
    function self.addStatus(name, value)
        if type(name) ~= 'string' then return end
        if type(value) ~= "number" then return end
        local status = self.status[name]
        if status then
            local newValue = status + value
            self.status[name] = newValue > 100 and 100 or ESX.Math.Round(newValue, 2)
        else
            self.status[name] = value
        end
        stateBag:set('status', self.status, true)
    ...
    ---@param name string
    ---@param value number
    function self.removeStatus(name, value)
        local status = self.status[name]
        if status then
            local newValue = status - value
            self.status[name] = newValue < 0 and 0 or ESX.Math.Round(newValue, 2)
        else
            self.status[name] = value
        end
        stateBag:set('status', self.status, true)
    ...
    --- Clear Player Status
    ---@param name string
    function self.clearStatus(name)
        if type(name) ~= 'string' then return end
