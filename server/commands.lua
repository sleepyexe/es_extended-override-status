...
ESX.RegisterCommand("refreshjobs", "superadmin", function()
    ESX.RefreshJobs()
end, true, { help = TranslateCap("command_clearall") })
...
ESX.RegisterCommand("saveall", "superadmin", function()
    Core.SavePlayers()
end, true, { help = TranslateCap("command_saveall") })

ESX.RegisterCommand("group", { "user", "admin", "superadmin" }, function(xPlayer, _, _)
    print(("%s, you are currently: ^5%s^0"):format(xPlayer.getName(), xPlayer.getGroup()))
end, true)

ESX.RegisterCommand("job", { "user", "admin", "superadmin" }, function(xPlayer, _, _)
    print(("%s, your job is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), xPlayer.getJob().name, xPlayer.getJob().grade_label))
end, false)

ESX.RegisterCommand("info", { "user", "admin", "superadmin" }, function(xPlayer)
